...
Markdown 到 HTML 的转换直接使用成熟库输出标准 HTML，再由浏览器截图。
...
- `--theme <NAME>`：可选，支持 `default`、`github-light`、`pico`、`splendor`。`default` 等同于 `github-light`。
...
- 标准 Markdown 标题、段落、粗体、斜体、删除线
- 引用、无序列表、有序列表、任务列表
- 行内代码、代码块、分隔线
- 链接、图片、表格、脚注
- 原始 HTML（按 Markdown 库默认行为原样输出）
- 内置主题：`github-light`、`pico`、`splendor`

## 当前限制

- 不支持 LaTeX / 非标准 Markdown 数学公式
- 不提供自定义 CSS、语法高亮、纯 Rust 渲染后端

仓库内部仍保留浏览器截图渲染抽象，未来可以补纯 Rust 后端。
